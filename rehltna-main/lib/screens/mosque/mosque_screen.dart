...
class _MosqueScreenState extends State<MosqueScreen> {
  ...
  Widget _buildDayCard(Map<String, dynamic> day) {
    ...
    return Container(
      width: 67,  // ✅ تقليل العرض من 72 إلى 65
      padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 4),  // ✅ تقليل الـ padding
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          Text(
            dayName,
            style: const TextStyle(fontSize: 11, color: Colors.white70),  // ✅ تصغير من 10 إلى 9
            textAlign: TextAlign.center,
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
          ),
          const SizedBox(height: 2),
          Text(icon, style: const TextStyle(fontSize: 20)),  // ✅ تصغير من 22 إلى 18
          const SizedBox(height: 2),
          Text(
            '$max°',
            style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.white),  // ✅ تصغير من 13 إلى 11
          ),
          Text(
            '$min°',
            style: const TextStyle(fontSize: 11, color: Colors.white60),  // ✅ تصغير من 11 إلى 9
          ),
          const SizedBox(height: 2),
          if (rain > 0)
            Text(
              '💧$rain%',
              style: const TextStyle(fontSize: 9, color: Colors.lightBlueAccent),  // ✅ تصغير من 9 إلى 7
            )
          else
            Text(
              '💨${wind.toStringAsFixed(0)}',
              style: const TextStyle(fontSize: 9, color: Colors.white54),  // ✅ تصغير من 9 إلى 7
            ),
        ],
      ),
    );
  ...
  Widget _buildCurrencyCard(bool isDark) {
    ...
    return Container(
      constraints: BoxConstraints(
        maxHeight: _currencyExpanded ? 500 : 72,
      ),
      decoration: BoxDecoration(
        color: bg,
  ...
  Widget _buildPrayerTimesCard(bool isDark) {
    ...
    final prayerItemHeight = 56.0;
    final maxPrayerHeight = prayerItemHeight * 7 + 120;

    return Container(
      constraints: BoxConstraints(
        maxHeight: _prayerExpanded ? maxPrayerHeight : 72,
      ),
      decoration: BoxDecoration(
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 14),
                const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('مواقيت الصلاة', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                ])),
                AnimatedRotation(
  ...
  Widget _buildTravelToolsCard(bool isDark) {
    ...
    return Container(
      constraints: BoxConstraints(
        maxHeight: _travelToolsExpanded ? 400 : 72,
      ),
      decoration: BoxDecoration(
        color: isDark ? const Color(0xFF1c2333) : Colors.white,
        borderRadius: BorderRadius.circular(20),
