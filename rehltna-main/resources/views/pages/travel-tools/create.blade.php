...
<div class="container-fluid" dir="rtl">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;margin-top:10px">
        ...
        <span style="font-weight:700;font-size:15px;color:#1e293b">إضافة أداة جديدة</span>
    </div>
    ...
    <form action="{{ route('travel-tools.store') }}" method="POST">
        @csrf
        <div class="form-card">
            <div class="row g-4">
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="icon" id="iconInput" class="form-control"
                           value="{{ old('icon') }}" placeholder="مثال: ✈️" maxlength="10"
                           style="font-size:24px;width:100px">
                    <div class="emoji-grid mt-2">
                        @foreach(['✈️','🧳','🗺️','🏨','🚢','🏖️','🏔️','🌍','💊','📋','🔐','💳','📱','🌐','⚡','🎫','🛂','🚗','🚂','🎒','🧭','⛽','🏥','🌡️','💰','📞','🔌','📷','🌤️','☀️','🕋'] as $e)
                            <span class="emoji-btn" onclick="setEmoji('{{ $e }}')">{{ $e }}</span>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="title" class="form-control"
                           value="{{ old('title') }}" placeholder="مثال: نصائح عند السفر" required>
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" class="form-control" rows="4"
                              placeholder="اكتب تفاصيل الأداة أو النصيحة هنا...">{{ old('description') }}</textarea>
                </div>
            ...
            <div class="mt-4 d-flex gap-3">
                <button type="submit" class="btn-save">💾 حفظ الأداة</button>
