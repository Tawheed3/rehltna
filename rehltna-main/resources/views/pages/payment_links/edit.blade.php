...
@section('content')
    ...
    <style>
        .select2-container .select2-selection--single { height: 38px !important; border: 1px solid #ced4da; display: flex; align-items: center; }
        .select2-container--default .select2-selection--single .select2-selection__arrow { height: 36px !important; }
        .item-row-box { background-color: #f8f9fa; border: 1px solid #e9ecef; border-radius: 8px; padding: 15px; margin-bottom: 15px; transition: all 0.3s ease; }
        .section-title { font-size: 1.1rem; font-weight: 600; color: #4a5568; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; margin-bottom: 20px; }
    </style>
    ...
    <div class="breadcrumb-header justify-content-between mb-4">
        <div class="my-auto">
            <div class="d-flex align-items-center">
                ...
                <span class="text-muted">Edit Payment Link</span>
            </div>
        </div>
    </div>
        ...
        <div class="col-xl-12">
            <div class="card shadow-sm">
                <div class="card-header bg-white pb-0">
                    <h4 class="card-title mb-0">Update Payment Link</h4>
                </div>
                <div class="card-body">
                    <form id="editForm" action="{{ route('payment-links.update', $link->id) }}" method="POST">
                        @csrf @method('PUT')
                        <input type="hidden" name="phone" id="full_phone" value="{{ old('phone', $link->phone) }}">
                        ...
                        <div class="mb-5">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="name" class="form-control" required value="{{ old('name', $link->name) }}">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label font-weight-bold">Email <span class="text-danger">*</span></label>
                                    <input type="email" name="email" class="form-control" required value="{{ old('email', $link->email) }}">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label font-weight-bold">Phone Number <span class="text-danger">*</span></label>
                                    <div class="row g-2">
                                        <div class="col-4 col-sm-3">
                                            <select id="phoneCodeSelect" class="form-control select2"><option value="" selected disabled>Code</option></select>
                                        </div>
                                        <div class="col-8 col-sm-9 pl-0">
                                            <input type="tel" id="phone_visual" class="form-control" placeholder="100 123 4567" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-3 mb-3">
                                    <label class="form-label font-weight-bold">Specialty</label>
                                    <input type="text" name="specialty" class="form-control" value="{{ old('specialty', $link->specialty) }}">
                                </div>
                                <div class="col-md-3 mb-3">
                                    <label class="form-label font-weight-bold">Country</label>
                                    <select id="countrySelect" name="country" class="form-control select2"><option value="" selected disabled>Select Country</option></select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-4 p-3 bg-light border rounded">
                            ...
                            <div class="d-flex gap-4">
                                <div class="form-check me-4">
                                    <input class="form-check-input" type="radio" name="link_type" id="type_items" value="items"
                                        {{ $link->items ? 'checked' : '' }}>
                                    <label class="form-check-label" for="type_items"><i class="fas fa-shopping-cart me-1"></i> Select Items/Courses</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="link_type" id="type_amount" value="amount"
                                        {{ $link->amount ? 'checked' : '' }}>
                                    <label class="form-check-label" for="type_amount"><i class="fas fa-dollar-sign me-1"></i> Custom Amount</label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="section-items">
                            ...
                            <div id="items-container">
                                @php
                                    $currentItems = is_string($link->items) ? json_decode($link->items, true) : $link->items;
                                    if(!$currentItems) $currentItems = [];
                                @endphp
                                @if(count($currentItems) > 0)
                                    @foreach($currentItems as $index => $savedItem)
                                        <div class="item-row item-row-box">
                                            <div class="row align-items-end">
                                                ...
                                                <div class="col-md-4 mb-2 mb-md-0">
                                                    ...
                                                    <input type="number" name="items[{{$index}}][attendees]" class="form-control"
                                                           value="{{ $savedItem['attendees'] ?? 1 }}" min="1" required>
                                                </div>
                                    ...
                                    @endforeach
                                @else
                                    {{-- Empty row template if user switches back to items --}}
                                    <div class="item-row item-row-box">
                                        <div class="row align-items-end">
                                            <div class="col-md-7 mb-2 mb-md-0">
                                                <label class="form-label small text-muted">Select Item</label>
                        ...
                        <div id="section-amount" style="display: none;">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <input type="number" name="amount" id="amount-input" class="form-control form-control-lg"
                                           step="0.01" placeholder="0.00" value="{{ $link->amount }}">
                                </div>
                                <div class="col-md-8">
                                    <label class="form-label font-weight-bold">Description / Note</label>
                                    <textarea name="note" id="note-input" class="form-control" rows="3" placeholder="Payment details...">{{ old('note', $link->note) }}</textarea>
                                </div>
                            </div>
                        </div>

                        <div class="d-flex justify-content-start mt-4 pt-3 border-top">
                            <a href="{{ route('payment-links.index') }}" class="btn btn-light btn-lg me-3">Cancel</a>
                            <button class="btn btn-primary btn-lg px-4" type="submit"><i class="fas fa-save mr-1"></i> Update Link</button>
                        </div>
                    </form>
                </div>
            </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#phoneCodeSelect').select2({ width: '100%' });
            ...
            // Trigger on load based on what's checked
            $('input[name="link_type"]:checked').trigger('change');

            // --- Dynamic Items Logic ---
            let itemIndex = 1000; // Safe index
            ...
            const phoneCodeSelect = $('#phoneCodeSelect');
            ...
            // Pre-fill Logic
            const savedCountry = "{{ old('country', $link->country) }}";
            if(savedCountry) countrySelect.val(savedCountry).trigger('change');

            const savedFullPhone = $('#full_phone').val();
            if (savedFullPhone) {
                let matchedCode = countriesData.find(c => savedFullPhone.startsWith(c.code));
                if (matchedCode) {
                    phoneCodeSelect.val(matchedCode.code).trigger('change');
                    $('#phone_visual').val(savedFullPhone.replace(matchedCode.code, ''));
                } else {
                    phoneCodeSelect.val('+20').trigger('change');
                    $('#phone_visual').val(savedFullPhone);
                }
            }

            $('#editForm').on('submit', function (e) {
                const code = phoneCodeSelect.val();
                const number = $('#phone_visual').val();
                if (!code || !number) {
